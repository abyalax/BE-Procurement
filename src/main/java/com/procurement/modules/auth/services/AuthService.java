...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        User user = User.builder().name(request.name().trim()).email(email)
                .password(passwordEncoder.encode(request.password())).status(UserStatus.ACTIVE)
                .roles(Set.of(userRole)).build();

        User savedUser = userRepository.save(user);

        String token = jwtService.generateToken(savedUser.getEmail(), roleNames(savedUser),
    ...
    private UserResponse toUserResponse(User user) {
        return new UserResponse(user.getId(), user.getName(), user.getEmail(), user.getStatus(),
                roleNames(user), permissionKeys(user), user.getCreatedAt(), user.getUpdatedAt());
    }

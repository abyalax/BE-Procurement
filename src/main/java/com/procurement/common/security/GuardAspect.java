...
public class GuardAspect {
  ...
  @Around(
    "@annotation(com.procurement.common.security.Guard) || @within(com.procurement.common.security.Guard)"
  )
  public Object guard(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    if (guard != null && guard.roles().length > 0 && !principal.hasAnyRole(guard.roles())) {
      throw new AccessDeniedException("Missing required roles: " + Arrays.toString(guard.roles()));
    }

    if (
      guard != null &&
      guard.permissions().length > 0 &&
  ...
  private JwtUserPrincipal resolvePrincipal() {
    ...
    if (
      authentication != null && authentication.getPrincipal() instanceof JwtUserPrincipal principal
    ) {
      return principal;
    }

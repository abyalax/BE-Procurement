...
public class SeederConfig {
    ...
    void seed() {
        ...
        for (Map.Entry<String, String> entry : permissions.entrySet()) {
            permissionRepository.findByKey(entry.getKey()).orElseGet(() -> permissionRepository
                    .save(Permission.builder().key(entry.getKey()).name(entry.getValue())
                            .description(entry.getValue()).build()));
        }

        Role adminRole = roleRepository.findByName("ADMIN")
                .orElseGet(() -> roleRepository.save(Role.builder().name("ADMIN")
                        .description("Full system administrator").build()));
        Role userRole = roleRepository.findByName("USER")
                .orElseGet(() -> roleRepository.save(Role.builder().name("USER")
                        .description("Default authenticated user").build()));
        ...
        if (!userRepository.existsByEmail(seedProperties.adminEmail())) {
            userRepository.save(User.builder().name("System Administrator")
